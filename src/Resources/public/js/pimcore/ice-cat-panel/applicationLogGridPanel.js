...
pimcore.registerNS('imcore.plugin.iceCatApplicationLogGridPanel.js');
pimcore.plugin.iceCatApplicationLogGridPanel = Class.create(pimcore.log.admin, {

      componentPrefix: 'ICECAT',

    initialize: function($super, configName) {
        $super({
            localMode: true,
            searchParams: {
                component: "ICECAT OBJECT-CREATION"
            }
        });
    },
    ...
    getTabPanel: function($super) {
        ...
        this.panel.setTitle(t('Log'));
        this.panel.setIconCls('pimcore_icon_log_admin');
        this.panel.setId('icecatApplicationLoggerPanel')
      
        // Hidding some column 
         this.resultpanel.columns[2].setFlex(80);
        this.resultpanel.columns[4].setHidden(true);
        this.resultpanel.columns[6].setHidden(true);
        this.resultpanel.columns[7].setHidden(true);  
