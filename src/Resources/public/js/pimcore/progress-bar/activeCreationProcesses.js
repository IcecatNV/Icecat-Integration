pimcore.registerNS("pimcore.plugin.IceCatActiveCreationProcesses");

pimcore.plugin.IceCatActiveCreationProcesses = Class.create(pimcore.plugin.admin, {
    displayList : [],
    toast : null,
    ...
    currentStep: 0,
    helper: null,
    objectGridTabSet: false,

    // activeItemIds: [],

    initialize: function (toastElement, type, parentCmp='processingProgressBarFieldset') {
        this.displayList = [];
    ...
    getPanelByItemId : function(id){
        return Ext.getCmp('processingProgressBar_active_process_'+id);
    },
    ...
    _buildBar: function(item) {
        ...
        if (this.getPanelByItemId(item.id)) {
            ...
            let progressBar = Ext.getCmp('processingProgressBar_monitoring_item_progress_bar_' + item.id);
            let actionBtnsPanel = Ext.getCmp('processingProgressBar_active_process_action_buttons_' + item.id);
        ...
        let progressBar = Ext.create('Ext.ProgressBar', {
            ...
            id: 'processingProgressBar_monitoring_item_progress_bar_' + item.id,
            alignOnScroll: false,
            height: 30,
            flex: 5,
        ...
        let headerText = {
            ...
            xtype: 'displayfield',
            alignOnScroll : false,
            value: item.name + ' (ID: <a href="#" class="process-manager-details-link" onclick="processingProgressBarPlugin.openLogId(' + item.id + ');  return false;">' + item.id +')</a>',
        ...
        let progressPanel = Ext.create('Ext.panel.Panel', {
            ...
            id: 'processingProgressBar_monitoring_item_progress_panel_' + item.id,
            flex: 12,
            layout: {
                type: 'hbox',
            },
        ...
        let panel = Ext.create('Ext.panel.Panel', {
            id: 'processingProgressBar_active_process_' + item.id,
    ...
    _updateProgressBar: function(progressBar, item) {
        ...
        let objectCreatedFlag = pimcore.globalmanager.get('objectCreatedFlag');

        if (Math.round(item.progressPercentage) == 100 && !this.objectGridTabSet) {
            let intervalObj = pimcore.globalmanager.get('iceCatIntervalObjCreation');
            if (intervalObj) {
                clearInterval(intervalObj);
                pimcore.globalmanager.remove('iceCatIntervalObjCreation')
            }
            this.helper.reactivateforNewProcess();
            pimcore.helpers.showNotification('Success', 'Products created Successfully!', 'Success');
            
            this.helper.showObjects();
            this.objectGridTabSet = true;           
        }
    },

    _updatePrimaryBtn: function(item) {
        let button = Ext.getCmp('processingProgressBar_monitoring_item_primary_button_' + item.id);
        if(item.isAlive){
        ...
        button.handler = function() {
            if(item.isAlive){
                Ext.Ajax.request({
                    url: '/admin/elementsprocessingProgressBar/monitoring-item/cancel',
                    method : 'get',
                    params : {
                        id : item.id
                    },
                ...
                Ext.Ajax.request({
                    url: '/admin/elementsprocessingProgressBar/monitoring-item/update',
                    method : 'post',
                    params : {
                        id : item.id,
                        published : true
    ...
    _updatePanelTitle : function(data){
        if(this.toast){
            //  let title = t('plugin_pm_process_list_title') + ' <small id="processingProgressBarActiveText">(' + data.active +' from  ' + data.total +' active)</small> ';
            //this.toast.setTitle(title);
            //do not use setTitle as it causes scrolling
            let el = document.getElementById('processingProgressBarActiveText');
            if(el){
                let s = '';
                if(data.active){
    ...
    requestServerData: function() {
        ...
        if(pimcore.globalmanager.get('objectCreatedFlag') == false)
        {
